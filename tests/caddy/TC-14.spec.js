...
test('test', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^AI$/ }).nth(2).click();
  await page.getByRole('button', { name: 'แจ้งปัญหา' }).click();
  await page.getByRole('textbox', { name: 'เลขหลุม' }).nth(2).click();
  await page.getByRole('textbox', { name: 'เลขหลุม' }).nth(2).fill('1');
  await page.getByPlaceholder('ระบุจำนวน 1 -').click();
  await page.getByPlaceholder('ระบุจำนวน 1 -').fill('1');
  await page.getByRole('button', { name: 'ยืนยัน' }).nth(2).click();
